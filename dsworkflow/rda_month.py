...
def get_filelist(yr, mth):
    ...
    for varclass in varsets:
        for var in varsets[varclass]:
            for day in days:
                firsthr = day.strftime("%Y%m%d00")
                lasthr = day.strftime("%Y%m%d23")
                fnpth = get_localpth(mthstr, firsthr, lasthr, varclass, var)
...
if __name__ == "__main__":
    ...
    print(f"Elapsed time: {int(run_time // 60)} minutes, {run_time % 60} seconds")
