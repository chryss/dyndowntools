...
Code that orchestrates all aspects of the ERA5/WRF dynamical downscaling process.

## Layout

 - `processing_code/`: the active orchestration code (formerly top-level `dsworkflow/`, then `pipeline/`)
...
This code is licensed under the [Creative Commons CC0 1.0 Universal License](https://creativecommons.org/publicdomain/zero/1.0/), as required by the U.S. Geological Survey for the companion Zenodo code publication — not the MIT license that applies to the rest of the repo. The full license deed is in `LICENSE.md`.
...
- **RDA/GDEX download URLs:** NCAR's Research Data Archive has changed its data-access endpoints multiple times thoughout this project (most recently with the August/September 2025 RDA→GDEX migration). The download scripts here (`rda_month.py`, `rda_JRA55_yr.py`) reflect the most recent known-working URLs as of this release — if downloads start failing, check the current access path for the relevant dataset ID at [gdex.ucar.edu](https://gdex.ucar.edu/datasets/).
- **HPC-specific paths:** the absolute paths throughout these scripts (e.g. `/center1/DYNDOWN/...`, `/import/SNAP/...`) and the `module load` calls are specific to the University of Alaska Fairbanks Research Computing Systems (UAF RCS) HPC system "Chinook." Running this code elsewhere requires adapting these paths and module names to the target system.
- **Source repository:** this folder is a curated subset of a larger monorepo published for the Zenodo code release. The full repository, including the rest of this project's analysis code, is at [github.com/chryss/dyndowntools](https://github.com/chryss/dyndowntools).
...
For a manually generated run, use `launch_wps_bridgeS.sh YYYYMM` to generate a WPS run for the `_B` folder for that year/month combination, and `launch_wps_monthS.sh YYYYMM` for the `_C` folder. These use `slurm` to submit a job to chinook. There are versions of these scripts without the `S` in `../dsworkflow_legacy/`, which run on the login node. But a) `metgrid.exe` is parallelized, so using SLURM represents a speedup and b) the process is very demanding in memory, so runnin multiple WPS processes on the login node in production isn't good practice.

The WPS scripts call `generate_namelists.py` to generate the correct namelist. 
...
This script will 
  ...
  - generate a namelist (using `generate_namelists.py`) and SLURM script
