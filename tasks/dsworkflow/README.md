...
## Layout
 ...
 - `processing_code/`: the active orchestration code 
 - `processing_code/config.sh`: instance-specific paths shared with bash scripts (see Notes below)
 - `processing_code/workflowutil.py`: shared Python utilities; also holds instance-specific values that aren't in `config.sh` (see Notes below)
...
- **RDA/GDEX download URLs:** NCAR's Research Data Archive has changed its data-access endpoints multiple times thoughout this project (most recently with the August/September 2025 RDA→GDEX migration). The download scripts here (`rda_month.py`, `rda_JRA_yr.py`) reflect the most recent known-working URLs as of this release — if downloads start failing, check the current access path for the relevant dataset ID at [gdex.ucar.edu](https://gdex.ucar.edu/datasets/).
- **HPC-specific paths:** instance-specific paths needed by bash scripts are centralized in `processing_code/config.sh`, `source`d directly by bash and read by Python via `python-dotenv`. Values that only Python needs — download URLs, the username for `squeue` lookups — are **not** in `config.sh`; they're plain constants near the top of `processing_code/workflowutil.py`. Adapting this pipeline to a different system or account means checking **both** files. `module load` calls are still hardcoded per-script, specific to the University of Alaska Fairbanks Research Computing Systems (UAF RCS) HPC system "Chinook". The `dyndown` conda env name refers to an environment generated by `dyndown_environment.yml`.
...
Two pipeline generations exist, split at 2024-01-26 — the first day covered by the "bridge" WPS folder for 202402 (see WPS folder timing below). All of January 2024 before that date still used GRIB ERA5 + JRA-55; that one month is transitional, split mid-month between the two pipelines rather than falling cleanly on one side.

#### NetCDF ERA5 (AWS mirror) + JRA-3Q — current, 2024-01-26 onward

JRA-3Q is downloaded once per campaign, ahead of any particular year's ERA5 download/preprocess — not on demand per year. From `scripts/`:
